...
class Egibest : MainAPI() {
    ...
    private fun isMovie(href: String, rawTitle: String): Boolean {
        ...
        return t.contains("فيلم") ||
            t.contains("film", ignoreCase = true) ||
    ...
    private fun Element.toCard(): SearchResponse? {
        ...
        val poster = img.absUrl("src").ifBlank { img.attr("src") }.takeIf { it.isNotBlank() }
        val rawTitle = (
    ...
    override val mainPage = mainPageOf(
        ...
        "$mainUrl/category/anime/?page=" to "أحدث الأنمي"
    )
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val items = doc.select("a.postBlockCol, a.postBlock, .postGrid a, .movie a")
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("a.postBlockCol, a.postBlock, .postGrid a, .movie a")
            .mapNotNull { it.toCard() }
            .distinctBy { it.url }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        doc.select("a.postBlock, div.postBlock a, .episodes a, .epAll a").forEach { ep ->
            ...
            val epPoster = ep.selectFirst("img")?.absUrl("src")
                ?.ifBlank { ep.selectFirst("img")?.attr("src") ?: "" }
